...
def get_HUC12_IDs_from_RhyessSys_Domain(XMin, YMin, XMax, YMax):
                ...
                if 'jobStatus' in jobJson:  
                    ...
                    if status == "esriJobFailed":                                        
                            if 'messages' in jobJson:                        
                                ...
                                print 'HydroTerre job failed get_HUC12_IDs_from_RhyessSys_Domain'
                                sys.exit(-100)
                                                   
        else:
            print "no HydroTerre jobId found in the response get_HUC12_IDs_from_RhyessSys_Domain"
            sys.exit(-101)
...
def main(argv):
    ...
    huc12s = get_HUC12_IDs_from_RhyessSys_Domain(XMin, YMin, XMax, YMax)
    print huc12s
...
def get_HUC12_IDs_from_RhyessSys_Domain(XMin, YMin, XMax, YMax):
	...
	check_interval=30
        taskUrl = "http://hydroterre.psu.edu:6080/arcgis/rest/services/RHESSys/getHUC12sfromRHESSysWorkflows/GPServer/get_HUC12s_from_RHESSysWorkflows"
